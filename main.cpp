...
static bool FindXorKey1Byte(const uint8_t* data, size_t size, uint8_t& bestKey, double& bestEntropy) {
    ...
    double origEntropy = XdrEntropy(data, size);
    bestKey = 0;
    bestEntropy = origEntropy;
    bool improved = false;
    std::vector<uint8_t> decrypted(size);
    AppendLog(L"[XDR] Recherche de clé XOR 1 octet... (taille=" + Dec(size) + L" octets, entropie initiale=" + std::to_wstring(origEntropy) + L")");
    const int totalKeys = 255;
    int processed = 0;
    int lastPercent = -1;
    for (int k = 1; k < 256; ++k) {
        for (size_t i = 0; i < size; ++i) decrypted[i] = data[i] ^ (uint8_t)k;
        double e = XdrEntropy(decrypted.data(), size);
        if (e < bestEntropy) {
            bestEntropy = e;
            bestKey = (uint8_t)k;
            improved = true;
            AppendLog(L"[XDR] Nouvelle meilleure clé: 0x" + Hex64(bestKey, 2) + L" (entropie=" + std::to_wstring(bestEntropy) + L")");
        }
        processed++;
        int percent = (processed * 100) / totalKeys;
        if (percent != lastPercent && (percent % 10 == 0 || percent == 100)) {
            UpdateProgress(percent, L"Recherche XOR " + std::to_wstring(percent) + L"%");
            AppendLog(L"[XDR] Progression recherche XOR : " + std::to_wstring(percent) + L"%");
            lastPercent = percent;
        }
    }
    bool found = improved && (origEntropy - bestEntropy) > 0.01;
    if (found) {
        AppendLog(L"[XDR] Clé XOR trouvée: 0x" + Hex64(bestKey, 2) + L" (entropie finale=" + std::to_wstring(bestEntropy) + L")");
    } else {
        AppendLog(L"[XDR] Aucune clé XOR significative trouvée (amélioration trop faible)");
    }
    return found;
}
...
static bool FindXorKey2Bytes(const uint8_t* data, size_t size, uint16_t& bestKey, double& bestEntropy) {
    const size_t sampleSize = std::min<size_t>(size, 1024 * 1024);
    std::vector<uint8_t> sample(data, data + sampleSize);
    double origEntropy = XdrEntropy(sample.data(), sampleSize);
    bestKey = 0;
    bestEntropy = origEntropy;
    bool found = false;
    AppendLog(L"[XDR] Recherche XOR 16 bits sur " + Dec(sampleSize) + L" octets...");
    ...
    for (int k = 1; k <= totalKeys; ++k) {
        ...
        if ((k % 1024) == 0) {
            UpdateProgress(10 + (k * 80 / totalKeys), L"XOR 16 bits...");
        }
    }
