...
#define IDM_TOOLS_XDR_CKDRM       1117
...
static bool GenerateXdrSummaryReport(const std::wstring& outputDir, const std::wstring& packerName, const PeInfo& pi) {
    std::wstring reportPath = outputDir + L"\\XDR_REPORT.md";
    std::wstringstream r;
    r << L"# ZennComp XDR-Protect - Rapport d'analyse\n\n";
    r << L"## Fichier analysé\n";
    r << L"- **Nom** : " << BaseName(g_targetPath) << L"\n";
    r << L"- **Taille** : " << Dec(g_file.size()) << L" octets\n";
    r << L"- **Packer détecté** : " << packerName << L"\n";
    r << L"- **Point d'entrée RVA** : 0x" << Hex64(pi.entryRva, 8) << L"\n";
    r << L"- **ImageBase** : 0x" << Hex64(pi.imageBase, pi.is64 ? 16 : 8) << L"\n\n";
    r << L"## Actions automatiques effectuées\n";
    r << L"- [x] Désassemblage des fonctions .pdata\n";
    r << L"- [x] Export ASM linéaire et récursif\n";
    r << L"- [x] Génération du pseudo-C\n";
    r << L"- [x] Export des sections données en C\n";
    r << L"- [x] Génération du script x64dbg (`zenncomp_dump_auto.dp64`)\n";
    r << L"- [x] Génération du guide d'unpacking (`UNPACK_GUIDE.txt`)\n\n";
    r << L"## Prochaines étapes recommandées\n";
    r << L"1. **Dump mémoire** avec x64dbg en suivant le guide.\n";
    r << L"2. **Reconstruire l'IAT** avec Scylla.\n";
    r << L"3. **Analyser le fichier décompressé** en le rechargeant dans ZennComp.\n";
    r << L"4. **Utiliser un unpacker spécifique** si disponible (mPRESS_unpacker.exe).\n\n";
    r << L"## Fichiers générés\n";
    std::vector<std::wstring> files = {
        L".text_linear.asm", L".text_recursive.asm", L".text_decompiled.c",
        L".text_cfg.dot", L".text_callgraph.dot",
        L".rdata_data.c", L".data_data.c", L".pdata_data.c",
        L"_RDATA_data.c", L"stkresv_data.c", L".rsrc_data.c", L".reloc_data.c",
        L"zenncomp_dump_auto.dp64", L"UNPACK_GUIDE.txt", L"XDR_REPORT.md"
    };
    for (const auto& f : files) {
        r << L"- `" << f << L"`\n";
    }
    return WriteUtf8TextFile(reportPath, r.str(), true);
}

// ---------- Scripts et rapports XDR ----------
static bool GenerateAdvancedX64dbgScript(const std::wstring& outputDir, uint64_t imageBase, uint32_t imageSize, const std::wstring& entryPointRva) {
    std::wstring scriptPath = outputDir + L"\\zenncomp_dump_auto.dp64";
    std::wstringstream script;
    script << L"// Script ZennComp - Dump automatique après déchiffrement\n";
    script << L"// Adapté pour MPRESS et autres packers\n";
    script << L"// Base: 0x" << Hex64(imageBase, 16) << L", Taille image: " << Dec(imageSize) << L"\n";
    script << L"// Point d'entrée: " << entryPointRva << L"\n\n";

    script << L"// Breakpoints sur les API de gestion mémoire\n";
    ...
    script << L"bp kernel32.VirtualFree\n\n";

    script << L"// Variable pour stocker l'adresse de la région décompressée\n";
    script << L"var dump_base\n";
    script << L"var dump_size\n\n";

    script << L"// Script de callback pour VirtualAlloc (exemple)\n";
    script << L"// Quand VirtualAlloc est appelé avec MEM_COMMIT | PAGE_READWRITE, on note l'adresse\n";
    script << L"// On peut aussi ajouter un breakpoint conditionnel si besoin\n";
    script << L"\n";
    script << L"// Exemple de commande pour dumper automatiquement après le retour de VirtualAlloc\n";
    script << L"// dump \"C:\\dump.bin\", $result, 0x1000000\n\n";

    script << L"// Pour utiliser Scylla automatiquement :\n";
    script << L"// scylla \"C:\\dump_auto.bin\", $result, 0x1000000\n\n";

    script << L"// Recommandations :\n";
    script << L"// 1. Exécuter le script (Run)\n";
    script << L"// 2. Quand un breakpoint est atteint, analyser la pile pour déterminer la taille\n";
    script << L"// 3. Utiliser la commande dump manuellement ou via un callback\n";
    script << L"// 4. Utiliser Scylla pour reconstruire l'IAT\n\n";

    script << L"// Exemple de callback pour dump automatique (à décommenter et adapter)\n";
    script << L"/*\n";
    script << L"bp VirtualAlloc\n";
    script << L"on bp VirtualAlloc\n";
    script << L"{\n";
    script << L"    // Récupérer le retour (adresse allouée)\n";
    script << L"    var addr = $result\n";
    script << L"    // Récupérer la taille (paramètre 2)\n";
    script << L"    var size = $arg2\n";
    script << L"    // Dumper si la taille est significative (> 1 Mo par exemple)\n";
    script << L"    if size > 0x100000\n";
    script << L"    {\n";
    script << L"        dump \"C:\\dump_region.bin\", addr, size\n";
    script << L"        log \"Dump automatique : 0x{addr} taille 0x{size}\"\n";
    script << L"    }\n";
    script << L"}\n";
    script << L"*/\n\n";

    script << L"// Pour MPRESS, souvent le stub appelle VirtualAlloc avec une grande taille,\n";
    script << L"// puis décompresse le code original à cette adresse.\n";
    script << L"// Une fois le déchiffrement terminé, on peut dumper la région entière.\n\n";

    script << L"run\n";
    script << L"\n";
    script << L"// Fin du script\n";

    bool ok = WriteUtf8TextFile(scriptPath, script.str(), false);
    if (ok) AppendLog(L"[OK] Script x64dbg avancé généré : " + scriptPath);
    return ok;
}

static bool GenerateUnpackerGuide(const std::wstring& outputDir, const PeInfo& pi, const std::wstring& packerName) {
    std::wstring guidePath = outputDir + L"\\UNPACK_GUIDE.txt";
    std::wstringstream guide;
    guide << L"============================================================\n";
    guide << L"  ZennComp - Guide d'analyse et d'unpacking pour " << packerName << L"\n";
    guide << L"============================================================\n\n";
    guide << L"Fichier analysé : " << g_targetPath << L"\n";
    guide << L"Point d'entrée RVA : 0x" << Hex64(pi.entryRva, 8) << L"\n";
    guide << L"ImageBase : 0x" << Hex64(pi.imageBase, pi.is64 ? 16 : 8) << L"\n\n";

    guide << L"=== Étape 1 : Dump mémoire avec x64dbg ===\n";
    guide << L"1. Ouvrir x64dbg (version 64 bits).\n";
    guide << L"2. Charger le fichier " << BaseName(g_targetPath) << L".\n";
    guide << L"3. Exécuter le script zenncomp_dump_auto.dp64 (Menu Script -> Run Script).\n";
    guide << L"4. Le script place des breakpoints sur VirtualAlloc, VirtualProtect, etc.\n";
    guide << L"5. Lancer l'exécution (F9).\n";
    guide << L"6. Quand le premier breakpoint est atteint, le stub de décompression est actif.\n";
    guide << L"7. Utiliser Scylla (plugin) :\n";
    guide << L"   - Cliquer sur 'Dump' pour sauvegarder l'image mémoire.\n";
    guide << L"   - Cliquer sur 'Fix Dump' pour reconstruire l'IAT.\n";
    guide << L"8. Le dump sauvegardé (généralement .exe ou .dll) peut être analysé avec ZennComp.\n\n";

    guide << L"=== Étape 2 : Utiliser un unpacker spécifique MPRESS ===\n";
    guide << L"Si vous disposez de mPRESS_unpacker.exe, vous pouvez l'utiliser :\n";
    guide << L"   mPRESS_unpacker.exe \"" << g_targetPath << L"\" \"" << outputDir << L"\\unpacked.exe\"\n";
    guide << L"Sinon, vous pouvez chercher un outil comme 'MPRESS Unpacker' ou 'QuickUnpack'.\n\n";

    guide << L"=== Étape 3 : Analyse des données exportées ===\n";
    guide << L"Les sections non-exécutables ont été exportées sous forme de tableaux C :\n";
    guide << L"   - " << SanitizeFileNamePart(L".rdata") << L"_data.c\n";
    guide << L"   - " << SanitizeFileNamePart(L".data") << L"_data.c\n";
    guide << L"   - etc.\n";
    guide << L"Vous pouvez les intégrer dans un projet de reconstruction.\n\n";

    guide << L"=== Étape 4 : Pseudo-C et CFG ===\n";
    guide << L"Le fichier .text_decompiled.c contient une représentation structurée du code.\n";
    guide << L"Les fichiers .dot (CFG, callgraph) peuvent être visualisés avec Graphviz.\n\n";

    guide << L"=== Recommandations ===\n";
    guide << L"- Pour MPRESS, le point d'entrée est souvent un stub qui décompresse le code original.\n";
    guide << L"- Les appels à VirtualAlloc/VirtualProtect se trouvent généralement près de l'EP.\n";
    guide << L"- Une fois décompressé, le code original a une entropie plus faible et peut être désassemblé.\n\n";

    guide << L"============================================================\n";
    bool ok = WriteUtf8TextFile(guidePath, guide.str(), false);
    if (ok) AppendLog(L"[OK] Guide d'unpacking généré : " + guidePath);
    return ok;
}
...
static std::wstring AnalyzeCurrentFile(bool includeStrings, bool includePseudo, bool includeDisasm) {
    ...
    // --- Calcul de la taille de l'image PE (utile pour le script x64dbg) ---
    ...
    // --- Détection packer et génération des rapports XDR (si packer trouvé) ---
    ...
    if (DetectPackerBySignatures(g_file.data(), g_file.size(), packerName)) {
        ...
        if (GetOutputDirChecked(dir, err2)) {
            // Script avancé x64dbg (4 arguments)
            GenerateAdvancedX64dbgScript(dir, pi.imageBase, imageSize, Hex64(pi.entryRva, 8));
            // Guide d'unpacking
            GenerateUnpackerGuide(dir, pi, packerName);
            // Rapport synthétique
            GenerateXdrSummaryReport(dir, packerName, pi);
        }
    }
...
static double PrintableRatioXor(const uint8_t* data, size_t len, uint8_t key) {
    if (!data || !len) return 0.0;
    size_t p = 0;
    for (size_t i = 0; i < len; ++i) {
        uint8_t c = data[i] ^ key;
        if (c == 0 || c == 9 || c == 10 || c == 13 || (c >= 0x20 && c <= 0x7E))
            p++;
    }
    return (double)p / (double)len;
}

static bool TryMpressUnpacker(const std::wstring& inputFile, const std::wstring& outputFile) {
    // Vérifier si mPRESS_unpacker.exe existe dans le PATH ou à côté de ZennComp
    wchar_t path[MAX_PATH];
    if (GetEnvironmentVariableW(L"PATH", path, MAX_PATH) == 0) return false;
    std::wstring pathStr = path;
    size_t pos = 0;
    while ((pos = pathStr.find(L';', pos)) != std::wstring::npos) {
        std::wstring dir = pathStr.substr(0, pos);
        std::wstring full = dir + L"\\mPRESS_unpacker.exe";
        if (GetFileAttributesW(full.c_str()) != INVALID_FILE_ATTRIBUTES) {
            // Lancer l'exécutable
            std::wstring cmd = L"\"" + full + L"\" \"" + inputFile + L"\" \"" + outputFile + L"\"";
            STARTUPINFOW si{ sizeof(si) };
            PROCESS_INFORMATION pi;
    ...
    // Vérifier à côté de l'exe
    wchar_t exePath[MAX_PATH];
    GetModuleFileNameW(nullptr, exePath, MAX_PATH);
    std::wstring exeDir = exePath;
    size_t lastSlash = exeDir.find_last_of(L"\\/");
    if (lastSlash != std::wstring::npos) exeDir = exeDir.substr(0, lastSlash + 1);
    std::wstring local = exeDir + L"mPRESS_unpacker.exe";
    if (GetFileAttributesW(local.c_str()) != INVALID_FILE_ATTRIBUTES) {
        std::wstring cmd = L"\"" + local + L"\" \"" + inputFile + L"\" \"" + outputFile + L"\"";
        STARTUPINFOW si{ sizeof(si) };
        PROCESS_INFORMATION pi;
        if (CreateProcessW(nullptr, &cmd[0], nullptr, nullptr, FALSE, CREATE_NO_WINDOW, nullptr, nullptr, &si, &pi)) {
            WaitForSingleObject(pi.hProcess, INFINITE);
...
static bool FindRotKey(const uint8_t* data, size_t size, uint8_t& bestRot, double& bestEntropy) {
    ...
    for (int rot = 1; rot < 8; ++rot) {
        std::vector<uint8_t> rotated(sampleSize);
        for (size_t i = 0; i < sampleSize; ++i) {
            rotated[i] = (sample[i] << rot) | (sample[i] >> (8 - rot));
        }
        double e = XdrEntropy(rotated.data(), sampleSize);
        if (e < bestEntropy) {
            bestEntropy = e;
            bestRot = (uint8_t)rot;
            found = true;
    ...
    return found && (origEntropy - bestEntropy) > 0.01;
}
...
static bool FindXorKey1Byte(const uint8_t* data, size_t size, uint8_t& bestKey, double& bestEntropy) {
    ...
    for (int k = 1; k <= totalKeys; ++k) {
        ...
        // Critère printabilité (arrêt précoce)
        double printableRatio = PrintableRatioXor(sample.data(), sampleSize, (uint8_t)k);
        if (printableRatio > 0.90) {
            bestKey = (uint8_t)k;
            bestEntropy = XdrEntropy(decrypted.data(), sampleSize);
            AppendLog(L"[XDR] Clé XOR 1 octet trouvée par printabilité : 0x" + Hex64(bestKey, 2) +
                      L" (taux=" + std::to_wstring(printableRatio * 100) + L"%)");
            return true;
        }
        ...
        // Progression + traitement des messages Windows
...
// Détection de DRM/packer par sections et imports
...
static bool FindXorKey2Bytes(const uint8_t* data, size_t size, uint16_t& bestKey, double& bestEntropy) {
    ...
    for (int k = 1; k <= totalKeys; ++k) {
        ...
        double printableRatio = (double)printable / (double)sampleSize;
        if (printableRatio > 0.90) {
            bestKey = (uint16_t)k;
            bestEntropy = XdrEntropy(decrypted.data(), sampleSize);
            AppendLog(L"[XDR] Clé XOR 16 bits trouvée par printabilité : 0x" + Hex64(bestKey, 4) +
                      L" (taux=" + std::to_wstring(printableRatio * 100) + L"%)");
            return true;
        }
        double e = XdrEntropy(decrypted.data(), sampleSize);
...
static void GenerateXdrArtifacts(const PeInfo& pi) {
    std::wstring dir, err;
    if (!GetOutputDirChecked(dir, err)) {
        AppendLog(L"[ERREUR] Impossible de créer le dossier de sortie pour XDR : " + err);
        return;
    }

    // Calcul de la taille de l'image PE
    uint32_t imageSize = 0;
    for (const auto& sec : pi.sections) {
        uint32_t end = sec.va + std::max(sec.vs, sec.rawSize);
        if (end > imageSize) imageSize = end;
    ...
    std::wstring entryRvaStr = Hex64(pi.entryRva, 8);

    // Appel avec 4 arguments (et non pi.imageSize)
    GenerateAdvancedX64dbgScript(dir, pi.imageBase, imageSize, entryRvaStr);
    GenerateUnpackerGuide(dir, pi, L"Fichier analysé");
    GenerateXdrSummaryReport(dir, L"Fichier analysé", pi);

    AppendLog(L"[XDR] Artefacts XDR générés dans " + dir);
}

static void AnalyzeEntryPoint(const PeInfo& pi, std::wstringstream& report) {
    if (pi.entryRva == 0) {
        report << L"[!] Aucun point d'entrée défini.\n";
        return;
    }
    report << L"\n## Analyse du point d'entrée (RVA 0x" << Hex64(pi.entryRva, 8) << L")\n";

    const SectionInfo* entrySec = nullptr;
    for (const auto& sec : pi.sections) {
        uint32_t start = sec.va;
        uint32_t end = sec.va + std::max(sec.vs, sec.rawSize);
        if (pi.entryRva >= start && pi.entryRva < end) {
            entrySec = &sec;
            break;
        }
    }
    if (!entrySec) {
        report << L"[-] Point d'entrée hors section connue.\n";
        return;
    }

    SectionBytes bytes;
    std::wstring reason;
    if (!ResolveSectionBytes(*entrySec, bytes, reason)) {
        report << L"[-] Impossible de lire la section : " << reason << L"\n";
        return;
    }
    uint64_t offsetInSec = (uint64_t)pi.entryRva - entrySec->va;
    if (offsetInSec + 1024 > bytes.size) {  // On agrandit à 1 Ko
        report << L"[-] Point d'entrée trop proche de la fin.\n";
        return;
    }
    const uint8_t* code = bytes.data + offsetInSec;
    size_t codeSize = std::min<size_t>(1024, bytes.size - offsetInSec);
    uint64_t entryVa = pi.imageBase + pi.entryRva;

    auto disasm = BuildDisassembly(code, codeSize, entryVa, pi.is64, 300, 5);
    if (disasm.empty()) {
        report << L"[-] Aucune instruction désassemblée.\n";
        return;
    }

    report << L"### Extraits du désassemblage de l'entrée\n";
    size_t shown = 0;
    for (const auto& kv : disasm) {
        report << L"`" << kv.second << L"`\n";
        if (++shown >= 150) break;
    }

    // Recherche d'appels à des API de gestion mémoire
    std::vector<std::wstring> apiCalls;
    for (const auto& kv : disasm) {
        std::wstring line = kv.second;
        std::wstring lower = LowerW(line);
        if (lower.find(L"virtualalloc") != std::wstring::npos ||
            lower.find(L"virtualprotect") != std::wstring::npos ||
            lower.find(L"virtualfree") != std::wstring::npos ||
            lower.find(L"ntallocatevirtualmemory") != std::wstring::npos ||
            lower.find(L"ntprotectvirtualmemory") != std::wstring::npos ||
            lower.find(L"rtlallocateheap") != std::wstring::npos ||
            lower.find(L"heapalloc") != std::wstring::npos) {
            apiCalls.push_back(line);
        }
    }
    if (!apiCalls.empty()) {
        report << L"\n### Appels à des API de gestion mémoire détectés :\n";
        for (const auto& call : apiCalls) {
            report << L"- " << call << L"\n";
            // Tenter d'extraire les paramètres (taille, etc.) pour aider au dump
            std::wstring lowerCall = LowerW(call);
            size_t pos = lowerCall.find(L"0x");
            if (pos != std::wstring::npos) {
                uint64_t val = ParseLastHexImmediate(call);
                if (val > 0x1000 && val < 0x100000000ULL) {
                    report << L"    Taille potentielle : 0x" << Hex64(val, 8) << L"\n";
                }
            }
        }
    } else {
        report << L"\nAucun appel direct à VirtualAlloc/VirtualProtect dans les 1024 premiers octets.\n";
    }
}
...
static void XdrAnalyzeSection(const SectionInfo& sec, const SectionBytes& bytes, const PeInfo& pi, std::wstringstream& report, bool forceSearch = true) {
    ...
    // --- Calcul de la taille de l'image PE (pour le script) ---
    uint32_t imageSize = 0;
    for (const auto& s : pi.sections) {
        uint32_t end = s.va + std::max(s.vs, s.rawSize);
        if (end > imageSize) imageSize = end;
    }

    // --- Détection packer sur l'ensemble du fichier (pas seulement la section) ---
    std::wstring packerName;
    AppendLog(L"[DEBUG] Appel de DetectPackerBySignatures sur g_file, taille=" + Dec(g_file.size()));
    if (DetectPackerBySignatures(g_file.data(), g_file.size(), packerName)) {
        AppendLog(L"[DEBUG] DetectPackerBySignatures a retourné TRUE, packer=" + packerName);
        report << L"[PACKER] " << packerName << L" détecté dans le fichier.\n";
        AppendLog(L"[XDR] Packer détecté : " + packerName);
        std::wstring dir, err;
        if (GetOutputDirChecked(dir, err)) {
            // --- Script avancé x64dbg ---
            if (GenerateAdvancedX64dbgScript(dir, pi.imageBase, imageSize, Hex64(pi.entryRva, 8))) {
                report << L"[OK] Script x64dbg avancé généré.\n";
            }
            // --- Guide d'unpacking ---
            if (GenerateUnpackerGuide(dir, pi, packerName)) {
                report << L"[OK] Guide d'unpacking généré.\n";
            }
            // --- Rapport synthétique ---
            if (GenerateXdrSummaryReport(dir, packerName, pi)) {
                report << L"[OK] Rapport XDR généré.\n";
            }
        } else {
            AppendLog(L"[ERREUR] Impossible d'obtenir le dossier de sortie pour les artefacts.");
        }
    }
    ...
    // --- Analyse point d'entrée si la section contient l'EP ---
    if (pi.entryRva >= sec.va && pi.entryRva < sec.va + std::max(sec.vs, sec.rawSize)) {
        AnalyzeEntryPoint(pi, report);
    }
    ...
    if (found1) {
        ...
        if (!disasmDecrypted.empty()) {
            std::wstring dir2, err2;
            if (GetOutputDirChecked(dir2, err2)) {
                std::wstring asmFile = dir2 + L"\\" + SanitizeFileNamePart(sec.name) + L"_decrypted_xor1.asm";
                ...
                if (!blocks.empty()) {
                    ...
                    std::wstring cFile = dir2 + L"\\" + SanitizeFileNamePart(sec.name) + L"_decrypted_xor1.c";
        ...
        report << L"[-] Aucune clé XOR 1 octet significative trouvée.\n";
    }
    ...
    if (found2) {
        ...
        // Optionnel : déchiffrer et exporter (code similaire à XOR1)
    } else {
    ...
    // --- Rotation ---
    uint8_t rotKey = 0;
    double bestEntRot = ent;
    if (FindRotKey(bytes.data, bytes.size, rotKey, bestEntRot)) {
        report << L"[+] Rotation gauche trouvée : " << (int)rotKey << L" bits (entropie=" << bestEntRot << L")\n";
    }
}
...
static bool FindXorKey4Bytes(const uint8_t* data, size_t size, uint32_t& bestKey, double& bestEntropy) {
    const size_t sampleSize = std::min<size_t>(size, 512 * 1024);
    std::vector<uint8_t> sample(data, data + sampleSize);
    double origEntropy = XdrEntropy(sample.data(), sampleSize);
    bestKey = 0;
    bestEntropy = origEntropy;
    bool found = false;
    AppendLog(L"[XDR-CKDRM] Recherche XOR 32 bits sur " + Dec(sampleSize) + L" octets (4,29 milliards de clés) - échantillonnage...");
    // Parcours limité à 2^24 clés pour rester raisonnable (16,7 millions)
    const uint32_t MAX_KEYS = 1 << 24;
    for (uint32_t k = 1; k < MAX_KEYS; ++k) {
        std::vector<uint8_t> decrypted(sampleSize);
        for (size_t i = 0; i < sampleSize; ++i) {
            decrypted[i] = sample[i] ^ ((k >> (8 * (i & 3))) & 0xFF);
        }
        double e = XdrEntropy(decrypted.data(), sampleSize);
        if (e < bestEntropy) {
            bestEntropy = e;
        ...
        if ((k % 0x10000) == 0) {
            int pct = (int)((uint64_t)k * 100 / MAX_KEYS);
            UpdateProgress(10 + (pct * 30 / 100), L"XOR 32 bits " + std::to_wstring(pct) + L"%");
            MSG msg;
            while (PeekMessageW(&msg, nullptr, 0, 0, PM_REMOVE)) {
                TranslateMessage(&msg);
                DispatchMessageW(&msg);
    ...
    if (found && (origEntropy - bestEntropy) > 0.05) {
        AppendLog(L"[XDR-CKDRM] Clé XOR 32 bits trouvée : 0x" + Hex64(bestKey, 8) + L" (entropie=" + std::to_wstring(bestEntropy) + L")");
        return true;
    }
    AppendLog(L"[XDR-CKDRM] Aucune clé XOR 32 bits significative.");
    return false;
}

static bool FindXorKey8Bytes(const uint8_t* data, size_t size, uint64_t& bestKey, double& bestEntropy) {
    const size_t sampleSize = std::min<size_t>(size, 512 * 1024);
    std::vector<uint8_t> sample(data, data + sampleSize);
    double origEntropy = XdrEntropy(sample.data(), sampleSize);
    bestKey = 0;
    bestEntropy = origEntropy;
    bool found = false;
    AppendLog(L"[XDR-CKDRM] Recherche XOR 64 bits sur " + Dec(sampleSize) + L" octets (échantillonnage sur 2^20 clés)...");
    // Limité à 1 million de clés pour éviter un temps infini
    const uint64_t MAX_KEYS = 1 << 20;
    for (uint64_t k = 1; k < MAX_KEYS; ++k) {
        std::vector<uint8_t> decrypted(sampleSize);
        for (size_t i = 0; i < sampleSize; ++i) {
            decrypted[i] = sample[i] ^ ((k >> (8 * (i & 7))) & 0xFF);
        }
        double e = XdrEntropy(decrypted.data(), sampleSize);
        if (e < bestEntropy) {
            bestEntropy = e;
        ...
        if ((k % 0x1000) == 0) {
            int pct = (int)(k * 100 / MAX_KEYS);
            UpdateProgress(40 + (pct * 30 / 100), L"XOR 64 bits " + std::to_wstring(pct) + L"%");
            MSG msg;
            while (PeekMessageW(&msg, nullptr, 0, 0, PM_REMOVE)) {
                TranslateMessage(&msg);
                DispatchMessageW(&msg);
    ...
    if (found && (origEntropy - bestEntropy) > 0.05) {
        AppendLog(L"[XDR-CKDRM] Clé XOR 64 bits trouvée : 0x" + Hex64(bestKey, 16) + L" (entropie=" + std::to_wstring(bestEntropy) + L")");
        return true;
    }
    AppendLog(L"[XDR-CKDRM] Aucune clé XOR 64 bits significative.");
    return false;
}

static bool BuildDecompressedPE(const PeInfo& pi, const std::vector<uint8_t>& decryptedData, const std::wstring& outputPath) {
    if (decryptedData.empty()) return false;
    // On crée un fichier PE minimal : on recopie les sections à leurs RVA
    // mais on ne reconstruit pas les headers correctement pour l'instant.
    // Dans un vrai cas, il faudrait reconstruire les headers NT, les sections, etc.
    // Ici on fait un dump binaire simple.
    HANDLE h = CreateFileW(outputPath.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
    if (h == INVALID_HANDLE_VALUE) {
        AppendLog(L"[ERREUR] Impossible de créer " + outputPath);
        return false;
    }
    DWORD wr = 0;
    WriteFile(h, decryptedData.data(), (DWORD)decryptedData.size(), &wr, nullptr);
    CloseHandle(h);
    AppendLog(L"[XDR-CKDRM] Dump déchiffré sauvegardé : " + outputPath);
    return true;
}

// Ajouter après la définition de BruteforceThreadParams
struct XdrCkdrmThreadParams {
    PeInfo pi;
    std::wstring result;
    bool finished = false;
    bool cancelled = false;
...
static std::wstring RunXdrCkdrmAnalysis(const PeInfo& pi) {
    std::wstringstream report;
    report << L"[XDR-CKDRM] Analyse en cours...\n";

    // Génération des artefacts XDR standards (on peut les garder ici ou les faire avant le thread)
    // Dans l'idéal, on les génère avant le thread pour ne pas ralentir le traitement.
    // On les laisse dans DoXdrCkdrmBruteforce pour que l'interface reste réactive.
    ...
    // Boucle sur les sections exécutables
    for (const auto& sec : pi.sections) {
        if (!(sec.characteristics & IMAGE_SCN_MEM_EXECUTE)) continue;
        SectionBytes bytes;
        if (!ResolveSectionBytes(sec, bytes, outErr)) continue;
        report << L"[XDR-CKDRM] Analyse de la section " << sec.name << L" (taille " << Dec(bytes.size) << L")\n";

        // Recherche de clés XOR
        uint8_t key1 = 0; double bestEnt1 = 0;
        uint16_t key2 = 0; double bestEnt2 = 0;
        uint32_t key4 = 0; double bestEnt4 = 0;
        uint64_t key8 = 0; double bestEnt8 = 0;

        bool found1 = FindXorKey1Byte(bytes.data, bytes.size, key1, bestEnt1);
        bool found2 = FindXorKey2Bytes(bytes.data, bytes.size, key2, bestEnt2);
        bool found4 = FindXorKey4Bytes(bytes.data, bytes.size, key4, bestEnt4);
        bool found8 = FindXorKey8Bytes(bytes.data, bytes.size, key8, bestEnt8);

        std::vector<uint8_t> bestDecrypted;
        double bestEntropy = XdrEntropy(bytes.data, bytes.size);
        std::wstring bestMethod = L"original";

        if (found1 && bestEnt1 < bestEntropy) {
            bestDecrypted.resize(bytes.size);
            for (size_t i = 0; i < bytes.size; ++i) bestDecrypted[i] = bytes.data[i] ^ key1;
            bestEntropy = bestEnt1;
            bestMethod = L"XOR1 (0x" + Hex64(key1, 2) + L")";
        }
        if (found2 && bestEnt2 < bestEntropy) {
            bestDecrypted.resize(bytes.size);
            for (size_t i = 0; i < bytes.size; ++i) bestDecrypted[i] = bytes.data[i] ^ ((key2 >> (8 * (i & 1))) & 0xFF);
            bestEntropy = bestEnt2;
            bestMethod = L"XOR2 (0x" + Hex64(key2, 4) + L")";
        }
        if (found4 && bestEnt4 < bestEntropy) {
            bestDecrypted.resize(bytes.size);
            for (size_t i = 0; i < bytes.size; ++i) bestDecrypted[i] = bytes.data[i] ^ ((key4 >> (8 * (i & 3))) & 0xFF);
            bestEntropy = bestEnt4;
            bestMethod = L"XOR4 (0x" + Hex64(key4, 8) + L")";
        }
        if (found8 && bestEnt8 < bestEntropy) {
            bestDecrypted.resize(bytes.size);
            for (size_t i = 0; i < bytes.size; ++i) bestDecrypted[i] = bytes.data[i] ^ ((key8 >> (8 * (i & 7))) & 0xFF);
            bestEntropy = bestEnt8;
            bestMethod = L"XOR8 (0x" + Hex64(key8, 16) + L")";
        }

        if (!bestDecrypted.empty()) {
            // Sauvegarde
            std::wstring outFile = outDir + L"\\" + SanitizeFileNamePart(sec.name) + L"_ckdrm_decrypted.bin";
            HANDLE h = CreateFileW(outFile.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
            if (h != INVALID_HANDLE_VALUE) {
                DWORD wr = 0;
                WriteFile(h, bestDecrypted.data(), (DWORD)bestDecrypted.size(), &wr, nullptr);
                CloseHandle(h);
                report << L"[XDR-CKDRM] Section déchiffrée sauvegardée : " << outFile << L" (méthode=" << bestMethod << L", entropie=" << std::to_wstring(bestEntropy) << L")\n";
            }

            // Désassemblage
            uint64_t base = pi.imageBase + sec.va;
            auto disasmDec = BuildDisassembly(bestDecrypted.data(), bestDecrypted.size(), base, pi.is64);
            if (!disasmDec.empty()) {
                auto blocks = BuildBasicBlocks(disasmDec);
                std::wstring pseudo = GeneratePseudoC(blocks, L"zc_" + SanitizeIdentifier(sec.name) + L"_ckdrm", true, true);
                std::wstring cFile = outDir + L"\\" + SanitizeFileNamePart(sec.name) + L"_ckdrm_decompiled.c";
                WriteUtf8TextFile(cFile, pseudo);
                report << L"[XDR-CKDRM] Pseudo-C généré pour la section déchiffrée : " << cFile << L"\n";
            }

            if (sec.name == L".text" || sec.name == L"UPX0" || sec.name == L"UPX1") {
                std::wstring peFile = outDir + L"\\" + SanitizeFileNamePart(sec.name) + L"_ckdrm_rebuilt.exe";
                BuildDecompressedPE(pi, bestDecrypted, peFile);
            }
        } else {
            report << L"[XDR-CKDRM] Aucune méthode de déchiffrement n'a amélioré l'entropie pour " << sec.name << L"\n";
        }
    }

    report << L"[XDR-CKDRM] Analyse terminée.\n";
    return report.str();
}

static unsigned int __stdcall XdrCkdrmThreadFunc(void* param) {
    _set_se_translator(SeTranslator);
    XdrCkdrmThreadParams* p = (XdrCkdrmThreadParams*)param;
    try {
        p->result = RunXdrCkdrmAnalysis(p->pi);
        p->finished = true;
    } catch (const std::exception& e) {
        p->result = L"[ERREUR] Exception C++ : " + Utf8ToWide(e.what()) + L"\r\n";
        p->finished = true;
...
static void DoXdrCkdrmBruteforce() {
    if (g_file.empty()) {
        MessageBoxW(g_hWnd, L"Aucun fichier chargé.", L"XDR-CKDRM", MB_ICONWARNING);
        return;
    }
    if (g_analysisRunning) {
        MessageBoxW(g_hWnd, L"Une analyse est déjà en cours.", L"XDR-CKDRM", MB_ICONINFORMATION);
        return;
    }
    ...
    ShowProgressWindow(L"XDR-CKDRM en cours");

    // On génère d'abord les artefacts XDR standards
    GenerateXdrArtifacts(pi);

    // Puis on lance une analyse plus poussée sur chaque section exécutable
    std::wstring outDir, outErr;
    if (!GetOutputDirChecked(outDir, outErr)) {
        AppendLog(L"[ERREUR] " + outErr);
    ...
    // Boucle sur les sections .text, .rdata, etc. pour tenter de déchiffrer
    for (const auto& sec : pi.sections) {
        if (!(sec.characteristics & IMAGE_SCN_MEM_EXECUTE)) continue;
        SectionBytes bytes;
        if (!ResolveSectionBytes(sec, bytes, outErr)) continue;
        AppendLog(L"[XDR-CKDRM] Analyse de la section " + sec.name + L" (taille " + Dec(bytes.size) + L")");

        // Recherche de clés XOR sur différentes tailles
        uint8_t key1 = 0; double bestEnt1 = 0;
        uint16_t key2 = 0; double bestEnt2 = 0;
        uint32_t key4 = 0; double bestEnt4 = 0;
        uint64_t key8 = 0; double bestEnt8 = 0;
        ...
        if (!bestDecrypted.empty()) {
            // Sauvegarder la section déchiffrée
            std::wstring outFile = outDir + L"\\" + SanitizeFileNamePart(sec.name) + L"_ckdrm_decrypted.bin";
            HANDLE h = CreateFileW(outFile.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
            if (h != INVALID_HANDLE_VALUE) {
                DWORD wr = 0;
                WriteFile(h, bestDecrypted.data(), (DWORD)bestDecrypted.size(), &wr, nullptr);
                CloseHandle(h);
                AppendLog(L"[XDR-CKDRM] Section déchiffrée sauvegardée : " + outFile + L" (méthode=" + bestMethod + L", entropie=" + std::to_wstring(bestEntropy) + L")");
            }

            // Tenter de désassembler la section déchiffrée
            uint64_t base = pi.imageBase + sec.va;
            auto disasmDec = BuildDisassembly(bestDecrypted.data(), bestDecrypted.size(), base, pi.is64);
            if (!disasmDec.empty()) {
                auto blocks = BuildBasicBlocks(disasmDec);
                ...
                std::wstring cFile = outDir + L"\\" + SanitizeFileNamePart(sec.name) + L"_ckdrm_decompiled.c";
                WriteUtf8TextFile(cFile, pseudo);
                AppendLog(L"[XDR-CKDRM] Pseudo-C généré pour la section déchiffrée : " + cFile);
            }

            // Si la section est la section principale, tenter de reconstruire un PE
            if (sec.name == L".text" || sec.name == L"UPX0" || sec.name == L"UPX1") {
                std::wstring peFile = outDir + L"\\" + SanitizeFileNamePart(sec.name) + L"_ckdrm_rebuilt.exe";
                BuildDecompressedPE(pi, bestDecrypted, peFile);
            }
        } else {
            AppendLog(L"[XDR-CKDRM] Aucune méthode de déchiffrement n'a amélioré l'entropie pour " + sec.name);
        }
    }

    g_analysisRunning = false;
    CloseProgressWindow();
    AppendLog(L"[XDR-CKDRM] Analyse terminée.");
    SetStatus(L"XDR-CKDRM terminé - " + BaseName(g_targetPath));
}
...
static void DoXdrBruteforce() {
    ...
    // --- GÉNÉRATION SYSTÉMATIQUE DES ARTEFACTS XDR ---
    GenerateXdrArtifacts(pi);
...
static void LayoutControls(HWND hwnd) {
    ...
    struct ButtonLayout { HWND h; int w; } ctrls[] = {
        ...
        {GetDlgItem(hwnd, IDC_BTN_XDR_CKDRM), 90}, 
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        AppendMenuW(menuTools, MF_STRING, IDM_TOOLS_XDR_CKDRM, L"XDR-CKDRM (décompression avancée)");
        ...
        btn(IDC_BTN_XDR_CKDRM, L"XDR-CKDRM");
    ...
    case WM_COMMAND: {
        ...
        case IDM_HELP_ETHICS:
            ...
            return 0;
        case IDC_BTN_XDR_CKDRM:
            DoXdrCkdrmBruteforce();
            return 0;
